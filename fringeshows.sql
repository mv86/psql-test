...
--SELECT name AND price FROM shows WHERE shows = SELECT MAX(price); 

--SELECT MAX(price) FROM  shows 

--SELECT (name, price) FROM SELECT MIN( price ) FROM shows WHERE price > ( SELECT MIN( price ) FROM shows )

--SELECT name, price FROM shows WHERE SELECT MIN(price) FROM shows WHERE price > (SELECT MAX(price) FROM shows);

--select name from users order by price limit 1 OFF 1;
